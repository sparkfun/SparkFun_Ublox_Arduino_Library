...
class SFE_UBLOX_GPS
{
  public:
	...
	boolean getTimeMode3(uint16_t maxWait = 250); //Get the current TimeMode3 settings
	boolean setTimeMode3(uint8_t mode, uint16_t observationTime, float requiredAccuracy, uint16_t maxWait = 250); //Begin Survey-In for NEO-M8P
	
	boolean getSVIN(uint16_t maxWait); //Reads survey in status and sets the global variables 
